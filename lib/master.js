...
class Master {
    ...
    this.wss.on('connection', (ws) => {
      ...
      ws.on('message', (messageString) => {
        ...
        if(message.type === 'rollback') {
          ...
          this.sendToClients({ event: 'DUMP_LOG', body: _.flatMap(this.log, _.identity) });
        } else if(message.type === 'add_filter') {
          let id = this.filter.add(message.body);
          this.sendToClients({ event: 'ADD_CENSORSHIP', body: message.body, id });
        } else if(message.type === 'remove_filter'){
          this.filter.remove(message.id);
          this.sendToClients({ event:'REMOVE_CENSORSHIP', id: message.id });
        } else if(message.type == 'export_filters') {
          let filters = this.filter.export();
          this.sendToClients({ event:'EXPORT_CENSORSHIP', filters });
        } else if(message.type === 'select') {
          this.removePool(message.sender, message.uid);
        }
      });
    });
  }
  ...
  sendToClients(data){
    ...
    console.log('[sendToClients]', dataString);
  ...
  rollbackState(time){
    ...
    // resending messages
    let resendingMessages = _.differenceBy(sended, received, 'uid');
  ...
  removePool(sender, uid){
    let index = this.messagePool.findIndex(msg => msg.sender === sender && msg.uid === uid );
