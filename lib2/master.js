...
class Master {
  ...
  rollbackState(time){
    ...
    for(let pidString in this.log){
      ...
      if(pid !== 0) {
        let lastUpdate = _.findLast(this.log[pid], e => e.event === 'ACTOR_UPDATED' ||
    ...
    // 非同期なのでreplace語になるようにし3
    let resendingMessages = _.differenceWith(sended, received, (a,b) => _.isEqual(a,b));
    resendingMessages.forEach((message) => {
      message.type = 'send';
      this.dispatch(message);
    });
  }
  ...
  removePool(uid){
    let index = this.messagePool.findIndex(msg => msg.uid === uid);
